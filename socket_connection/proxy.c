...
/* Mia Lassiter
Web proxy 
Instructions: change your browser configuration to use a proxy. 
All HTTP requests using port 80 will be sent to the proxy 
...
void *connectionHandler(void *sock) {
    ...
    while((n = read(clientfd, response, sizeof(response))) > 0) {
        ...
        fputs(response, stdout); //print response
        assert(bytes_sent != -1);
