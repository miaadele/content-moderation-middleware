...
for container in containers:
    ...
    post_date = get_text(
        container, "div", {"class": "ml4 mt2 text-body-xsmall t-black--light"}
    )
    ...
    comment_element = container.find_all(
        lambda tag: tag.name == "button"
        and "aria-label" in tag.attrs
        and "comment" in tag["aria-label"].lower()
    )
    comment_idx = 1 if len(comment_element) > 1 else 0
    post_comments = (
        comment_element[comment_idx].text.strip()
    ...
    shares_element = container.find_all(
        lambda tag: tag.name == "button"
        and "aria-label" in tag.attrs
        and "repost" in tag["aria-label"].lower()
    )
    shares_idx = 1 if len(shares_element) > 1 else 0
    post_shares = (
        shares_element[shares_idx].text.strip()
